...
// END PA 4 FUNCTIONS
  ...
  private:
    ...
    /*
    *Helper method for buildTree recursive call
    */
    QuadtreeNode* helperbuildTree(QuadtreeNode* nd, int resolution, PNG const& source, int x, int y);

    /*
    *Helper method to calculate the RGBA value
    */
    RGBAPixel calculatingAverageRGBA(const QuadtreeNode* nd);

    /**
    *This method computes the average red RGBA for a parent node
    */
    int averageRed(const QuadtreeNode* nd);

    /**
    *This method computes the average blue RGBA for a parent node
    */
    int averageBlue(const QuadtreeNode* nd);

    /**
    *This method computes the average green RGBA for a parent node
    */
    int averageGreen(const QuadtreeNode* nd);

    /**
    *This method computes the average alpha RGBA for a parent node
    */
    int averageAlpha(const QuadtreeNode* nd);

    /**
    *Helper method for deep copy
    */
    QuadtreeNode*  deepCopyHelper(const QuadtreeNode* other);

    /**
    *Helper method for deconstructor and deleteTree
    */
    void deleteTreeHelper( QuadtreeNode* nd);

    /**
    *Helper method for getPixel
    */

    RGBAPixel getPixelHelper( QuadtreeNode* nd,int x, int y, int resolution) const;

    /**
    *Helper method for clockwise rotation
    *Does rotation by pointer manipulation
    */
    void clockwiseRotateHelper( QuadtreeNode* nd);

    /**
    *This method returns the height of the tree
    *Used for testing purpose only
    */
    int treeHeight( QuadtreeNode* nd) const;

    /**
    *Helper method for calculating quadtree's height
    */
    int max(int a, int b) const;

    //Helper method for decompressing the image
    void helperDecompress(PNG& png, QuadtreeNode* curr, int x, int y, int resolution) const;


    //helper method for prune
    //this method deletes the children of the node if the tolerance condition is met, otherwise keep prunning from root to leaves
    void helpPrune(QuadtreeNode* root, int tolerance);

    // helper method for helpPrune
    //returns true if the node meets the tolerance condition
    //returns false if the node doesn't meet the tolerance condition
    bool prunable(QuadtreeNode* root, QuadtreeNode* curr, int tolerance);
    ...
    bool diffPrune(QuadtreeNode* parent, QuadtreeNode* child, int tolerance);

    /**
    *Helper method to count pruneSize
    */
    int helpPruneSize( QuadtreeNode* nd, int tolerance) const;

    //helper function to find the pruneSize
    //This method behave similarly with prunable, except for instead of deleting the node, it counts how many remaining nodes will be there you are prunning the image
    //Doesn't change the state of the root, returning the number of leaves remaining after prune
    bool prunableSize(QuadtreeNode* nd, QuadtreeNode* curr, int tolerance) const;

    //helper method to determine if color value of no leaf in Tn differs from
    //the component-wise average color value, a, by no more than tolerance
    bool diffPruneSize(QuadtreeNode* parent, QuadtreeNode* child, int tolerance) const;

    //The binary search to look for the values for tolerance given the number of leaves remaining after prunning
    //Returns the integer of the minimum tolerance
    int binarySearch(int min, int max, int numLeaves)const;


    //The helper method for binary search
    //This method is called when binary search meets its first idealLeaves number, that is not necessarily the minimum value.
    //This method keeps track of the smallest value for tolerance and return the minimum tolerance value
    int binarySearchMinimumTolerance(int min, int max, int numLeaves, int similar) const;
